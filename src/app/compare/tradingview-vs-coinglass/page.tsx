...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: "TradingView vs CoinGlass: Best Crypto Charting & Data Platform Compared | CryptoFinder",
    description: "TradingView is the undisputed king of charting. CoinGlass is the go-to for derivatives data. This comparison helps you decide which tool belongs in your stack.",
    keywords: ["TradingView", "CoinGlass", "crypto charting", "futures data", "comparison"],
    openGraph: {
      title: "TradingView vs CoinGlass | CryptoFinder",
      description: "Charting king vs derivatives data powerhouse — head-to-head comparison.",
      url: "https://cryptoaifinder.com/compare/tradingview-vs-coinglass",
      type: "article",
    },
    alternates: {
      canonical: "https://cryptoaifinder.com/compare/tradingview-vs-coinglass",
    },
  };
}
...
export default async function ComparePage({ params }: PageProps) {
  const { slug: _slug } = await params;
  const tool1 = tools.find(t => t.id === "tradingview");
  const tool2 = tools.find(t => t.id === "coinglass");
    ...
    <main className="min-h-screen bg-slate-900 text-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold mb-4">
          TradingView <span className="text-cyan-400">vs</span> CoinGlass
        </h1>
        <p className="text-slate-400 mb-8 text-lg">
          TradingView is the undisputed king of charting with its Pine Script ecosystem and social community. CoinGlass, on the other hand, is the go-to platform for derivatives traders — liquidation heatmaps, open interest, and funding rates. This comparison helps you decide which tool (or both) belongs in your trading stack.
        </p>
            ...
            <div>
              ...
              <ul className="space-y-2 text-slate-300 text-sm">
                <li className="flex items-start gap-2">
                  ...
                  <span>You rely on advanced charting with custom Pine Script indicators</span>
                </li>
                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-400 mt-0.5 shrink-0" />
                  <span>You want a social community and idea-sharing platform</span>
                </li>
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-2 text-slate-300 text-sm">
                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-400 mt-0.5 shrink-0" />
                  <span>You trade futures and need liquidation heatmaps + OI data</span>
                </li>
                <li className="flex items-start gap-2">
                  <Check className="w-4 h-4 text-green-400 mt-0.5 shrink-0" />
                  <span>You want funding rate tracking across multiple exchanges</span>
                </li>
              </ul>
            </div>
          </div>
